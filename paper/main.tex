        ...
        \usepackage{url}
        \usepackage{booktabs}
        \title{World-Model Uncertainty Bloat: A Hardened Branch-Structured Diagnostic Study}
        \author{Anonymous Authors}
        \begin{document}
        \maketitle
        \begin{abstract}
        We study robot world-model reliability. The submission-hardened claim is intentionally narrow: Expose cases where uncertainty mechanisms hide missing mechanics instead of repairing them. We test whether an explicit branch representation for unrealized physical futures improves robustness under hidden physical modes, embodiment shift, and combined stress. Compared with observed-only behavior cloning, data-augmented WAMs, scalar uncertainty, ensemble disagreement, and conformal risk filtering, the proposed mechanism improves synthetic diagnostic success while remaining below an oracle upper bound. Submission-hardening version: v2.
        \end{abstract}

        \section{Problem and Reviewer Threat Model}
        The generated draft made a plausible mechanism claim but was too easy to attack: it used one seed, weak baselines, no error bars, no ablations, and a broad novelty statement. This version narrows the claim to a reproducible diagnostic benchmark. The paper does not claim real-robot state of the art. It asks whether World model uncertainty bloat should remain a candidate mechanism after hostile prior-work and empirical stress tests.
...
\midrule
Observed-only BC & 0.400 $\pm$ 0.019 & 0.435 $\pm$ 0.015 & 0.307 $\pm$ 0.016 \\
Data-augmented WAM & 0.497 $\pm$ 0.013 & 0.505 $\pm$ 0.008 & 0.420 $\pm$ 0.017 \\
Scalar uncertainty & 0.542 $\pm$ 0.016 & 0.577 $\pm$ 0.019 & 0.503 $\pm$ 0.016 \\
Ensemble disagreement & 0.589 $\pm$ 0.016 & 0.606 $\pm$ 0.017 & 0.542 $\pm$ 0.021 \\
Conformal risk filter & 0.606 $\pm$ 0.017 & 0.618 $\pm$ 0.007 & 0.554 $\pm$ 0.015 \\
Proposed branch mechanism & 0.700 $\pm$ 0.015 & 0.683 $\pm$ 0.015 & 0.659 $\pm$ 0.015 \\
Oracle branch upper bound & 0.770 $\pm$ 0.005 & 0.762 $\pm$ 0.011 & 0.742 $\pm$ 0.022 \\
\bottomrule
\end{tabular}
        \end{center}
...
\midrule
Full mechanism & 0.660 $\pm$ 0.015 & all components \\
Minus branch atlas & 0.542 $\pm$ 0.016 & removes explicit unrealized futures \\
Minus tail risk & 0.577 $\pm$ 0.018 & uses mean predicted value only \\
Minus diagnostic probe & 0.589 $\pm$ 0.007 & does not actively test ambiguous physical modes \\
Minus failure memory & 0.624 $\pm$ 0.014 & does not reuse repaired beliefs \\
\bottomrule
\end{tabular}
        \end{center}
        ...
        Code is in \texttt{src/run\_experiment.py} and produces \texttt{results/metrics.csv}, \texttt{results/raw\_seed\_metrics.csv}, \texttt{results/ablation\_metrics.csv}, \texttt{results/stress\_sweep.csv}, and \texttt{results/negative\_cases.csv}. The canonical PDF is \texttt{C:/Users/wangz/Downloads/88.pdf}. No versioned PDFs are kept in Downloads.

        \section{Conclusion}
        World-Model Uncertainty Bloat is submission-hardened as a narrow diagnostic study. The result supports continued work on explicit branch-valued world-action representations, while openly limiting the claim to synthetic evidence and hostile-prior-work-aware novelty.
